$(document).ready(function() {
    ...
    $(document).on('submit', '#form', function(ev) {
        ...
        if (qte != '' && id_prod != '' && prix != '') {
            ...
            $('#msg').html('<div class="alert alert-danger">Remplissez tous les champs</div>');
        }
    });
    ...
    $(document).on('submit', '#valider', function(ev) {
        ...
        var date_retrait = $('#date_retrait').val();

        if (pt != '' && total != '' && id_client != '' && montant != '' && modele != '' && date_retrait != '') {
            var data = $(this).serialize();
            $.ajax({
                url: "validation_commandes.php",
                method: "POST",
                data: data,
                dataType: "JSON",
                success: function(response) {
            ...
            $('#message').html('<div class="alert alert-danger">Validation impossible, la commande est vide</div>');
        }
    });
    ...
    $(document).on('submit', '#formClient', function(e) {
        ...
        if (noms_client != '' && prenoms_client != '' && num_client != '' && ville != '') {
            ...
            $('#sms').html('<div class="alert alert-danger">Remplissez tous les champs</div>');
        }
    });
});