<?php
    ...
    $requette=$db->query("SELECT SUM(details_recette.pt) AS total_g FROM details_recette JOIN  recette ON details_recette.id_recette=recette.id WHERE recette.annee_recette='$annee' ");
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <h5 class="text-center mb-0">TABLEAUX RECAPITULATIFS DES VENTES DE L'ANNEE <?php echo $annee; ?></h5>
                    </div>
                    <div class="card-body">
                    ...
                    <?php  
                      ...
                      $tt=$db->query("SELECT  details_recette.libelle, SUM(details_recette.qte) AS qte, SUM(details_recette.pu) AS pu, SUM(details_recette.pt) AS pt FROM details_recette JOIN categories ON details_recette.id_cat=categories.id_cat JOIN recette ON details_recette.id_recette=recette.id WHERE details_recette.id_cat=$data->id_cat AND recette.annee_recette='$annee' GROUP BY details_recette.libelle");
                      $fd = $tt->fetchAll(PDO::FETCH_OBJ);
                      $ligne= $tt->rowCount();

                      $requet=$db->query("SELECT SUM(details_recette.pt) AS total FROM details_recette JOIN categories ON details_recette.id_cat=categories.id_cat JOIN recette ON details_recette.id_recette=recette.id WHERE details_recette.id_cat=$data->id_cat AND recette.annee_recette='$annee' ");
