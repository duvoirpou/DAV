<?php
    ...
    $requette=$db->query("SELECT  SUM(stock) AS ts, SUM(entree)AS te, SUM(sortie) AS tsort, SUM(solde) AS tsolde FROM stock WHERE  mois_st='$mois' ");
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <h5 class="text-center mb-0">TABLEAUX RECAPITULATIFS DU STOCK DU MOIS DE <?php echo $mois; ?></h5>
                    </div>
                    <div class="card-body">
                    ...
                    <?php  
                      ...
                      $tt=$db->query("SELECT matiere_premiere.libelle, categories.libelle_cat, SUM(stock.stock) AS stock, SUM(stock.entree)AS entree, SUM(stock.sortie) AS sortie,SUM(stock.solde) AS solde FROM stock JOIN matiere_premiere ON stock.id_mat=matiere_premiere.id_mat JOIN categories ON stock.id_cat=categories.id_cat WHERE stock.id_cat=$data->id_cat AND stock.mois_st='$mois' GROUP BY matiere_premiere.libelle");
                      $fd = $tt->fetchAll(PDO::FETCH_OBJ);
                      $ligne= $tt->rowCount();

                      $requet=$db->query("SELECT  SUM(stock) AS ts, SUM(entree)AS te, SUM(sortie) AS tsort, SUM(solde) AS tsolde FROM stock WHERE id_cat=$data->id_cat AND mois_st='$mois' ");
                    ...
                    <div class="text-center"><a class="btn btn-success" href="print_rec_mois.php?m=<?= $mois ?>" target="_blank"><i class="fa fa-print"></i> Imprimer</a></div>
